  ...
  'serviceWorker' in navigator &&
    window.addEventListener('load', () => {
      navigator.serviceWorker
        ...
        .then((n) => {
          (registration = n), InitNoticeUI();
        });
    });
  sessionStorage.Page === undefined && (sessionStorage.Page = MENU1);
  DetectOnLine();
  SwitchPage();
}
function InitNoticeUI() {
  registration !== null &&
    ...
    registration.pushManager.getSubscription().then(function (n) {
      isSubscribed = !(n === null);
      SubscribeNotice();
    });
}
function IsNull(n) {
  return n === null || typeof n == 'undefined' || n === '' || n === 'null';
}
function Loading(n) {
  switch (n) {
    case !0:
...
function UrlB64ToUint8Array(n) {
  const u = '='.repeat((4 - (n.length % 4)) % 4),
    f = (n + u).replace(/\-/g, '+').replace(/_/g, '/'),
    i = window.atob(f),
    r = new Uint8Array(i.length);
  for (var t = 0; t < i.length; ++t) r[t] = i.charCodeAt(t);
  return r;
}
const MENU1 = 'Welcome',
  MENU2 = 'Intro',
...
$('.js-side-nav').click(function () {
  $(event.target).hasClass('visible') === !0 &&
    $('.js-side-nav').toggleClass('visible');
});
